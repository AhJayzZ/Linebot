...
bot.on('message', function(event) {
    ...
    if (event.message.type = 'text') {
        ...
        if (msg.search('你好') >= 0)
            event.reply('好你娘及掰啦');
        else if (msg.search('哈') >= 0)
            event.reply('哈殺小,去哈龜啦');
        else if (msg.search('早安') >= 0)
            event.reply('你媽死了知道不?');
        else if (msg.search('==') >= 0 || msg.search('= =') >= 0) {
            angry_count = angry_count + 1;
            event.reply('誰在打==或= =試看看啦，憤怒指數:' + angry_count);
        } else if (msg.search('可憐') >= 0)
            event.reply('你全家才可憐');

        else if (msg.search('祖先') >= 0) {
            mymsg = {
                type: 'image',
            ...
            event.reply(mymsg);
        } else if (msg.search('QQ') >= 0)
            event.reply('Q你媽逼');

        else if (msg.search('!') >= 0)
            event.reply('我看不懂啦QQ');
    }
